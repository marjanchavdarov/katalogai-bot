...
print(f"  TWILIO_TOKEN   : {'✅ SET' if TWILIO_TOKEN else '❌ MISSING'}")
print("=" * 60)
...
def get_user(phone):
    try:
        r = requests.get(
            f"{SUPABASE_URL}/rest/v1/users?phone=eq.{requests.utils.quote(phone)}&select=*&limit=1",
            headers=supa_headers(), timeout=10
        )
        data = r.json()
        return data[0] if data else None
    except Exception as e:
        print(f"⚠️ get_user exception: {e}")
        return None
...
def create_user(phone):
    ...
    try:
        ...
        print(f"⚠️ create_user exception: {e}")
        return record


def update_user_activity(phone):
    try:
        requests.patch(
            f"{SUPABASE_URL}/rest/v1/users?phone=eq.{requests.utils.quote(phone)}",
            headers=supa_headers({"Prefer": "return=minimal"}),
            json={"last_active": datetime.utcnow().isoformat(), "total_searches": None},
            timeout=10
        )
    except:
        pass
...
# ─────────────────────────────────────────────
STORES = ["plodine", "lidl", "kaufland", "konzum", "spar", "studenac"]
STORE_MAP = {s: s.capitalize() for s in STORES}
STORE_MAP["lidl"] = "Lidl"
STORE_MAP["kaufland"] = "Kaufland"
STORE_MAP["konzum"] = "Konzum"
STORE_MAP["spar"] = "Spar"
STORE_MAP["plodine"] = "Plodine"
STORE_MAP["studenac"] = "Studenac"
...
STOP_WORDS = {
    ...
    "tell","list","all","from","store","shop","market","supermarket","catalog",
    "catalogue","katalog","akcija","cijena","gdje","ima","daj","molim","trazi",
    "kakav","ima","imas","mi","da","li","po","na","je","su","se","za","od","do"
}

def search_products(message, max_results=25):
    """Smart filtered product search — never loads all products."""
    msg_lower = message.lower()

    # Detect store
    detected_store = next((STORE_MAP[s] for s in STORES if s in msg_lower), None)

    # Extract meaningful keywords
    words = [w for w in re.findall(r'[a-zA-ZčćđšžČĆĐŠŽ]+', msg_lower)
             if len(w) > 2 and w not in STOP_WORDS and w not in STORES]

    # Build Supabase filter URL
    base = f"{SUPABASE_URL}/rest/v1/products"
    ...
    if detected_store:
        params.append(f"store=eq.{detected_store}")

    if words:
        keyword = words[0]
        params.append(f"product=ilike.*{keyword}*")

    params.append(f"limit={max_results}")
    params.append("order=discount_percent.desc")
    ...
    try:
        ...
        print(f"  🔍 Search '{message[:50]}' → store={detected_store}, keyword={words[0] if words else 'none'}, results={len(products)}")
        return products, detected_store
    except Exception as e:
        print(f"⚠️ search_products exception: {e}")
        return [], detected_store


def search_catalogues(store=None):
    """Get available catalogues, optionally filtered by store."""
    params = ["select=store,catalogue_name,valid_from,valid_until,pages,products_count",
              "order=valid_from.desc", "limit=10"]
    if store:
        params.append(f"store=eq.{store}")

    url = f"{SUPABASE_URL}/rest/v1/catalogues?" + "&".join(params)
    try:
        r = requests.get(url, headers=supa_headers(), timeout=10)
...
def get_page_image(store, page_number):
    """Get page image URL for a specific store and page."""
    store_cap = STORE_MAP.get(store.lower(), store)
    url = (f"{SUPABASE_URL}/rest/v1/products?"
           f"select=page_image_url,page_number,store&"
           f"store=eq.{store_cap}&"
           f"page_number=eq.{page_number}&"
           f"is_expired=eq.false&limit=1")
    try:
        r = requests.get(url, headers=supa_headers(), timeout=10)
        data = r.json()
        if data and isinstance(data, list) and data[0].get("page_image_url"):
            return data[0]["page_image_url"]
        return None
    except:
        return None
...
# ─────────────────────────────────────────────
# CHAT WITH GEMINI (text only, no vision)
# ─────────────────────────────────────────────
GEMINI_CHAT_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent"

SYSTEM_PROMPT = """You are katalog.ai, a smart multilingual shopping assistant.
You help users find the best supermarket deals from weekly catalogs in Croatia.

ALWAYS respond in the SAME language the user writes in.
NEVER use markdown, asterisks, bold or special formatting.
Keep responses short, friendly and useful.
Maximum 8 results per response.

Available stores: Lidl, Kaufland, Konzum, Spar, Plodine, Studenac

When showing products use this format:
Product name - Price EUR (Store, valid until DATE)

If no products found, say so clearly and suggest alternatives."""


def chat_with_gemini(user_message, products, catalogues, history, language="hr"):
    """Send message + search results to Gemini. Returns text reply."""
    if not GEMINI_API_KEY:
        return "Servis trenutno nije dostupan. / Service currently unavailable."

    # Build context from search results
    context_parts = []

    if products:
        context_parts.append(f"SEARCH RESULTS ({len(products)} products found):")
        for p in products[:20]:
            line = f"- {p.get('product','')} {p.get('brand','')} {p.get('quantity','')} | {p.get('sale_price','?')} EUR"
            if p.get('original_price'):
                line += f" (was {p.get('original_price')} EUR)"
            if p.get('discount_percent'):
                line += f" -{p.get('discount_percent')}"
            line += f" | {p.get('store','')} | valid until {p.get('valid_until','')} | page {p.get('page_number','')}"
            context_parts.append(line)

    if catalogues:
        context_parts.append(f"\nAVAILABLE CATALOGUES:")
        for c in catalogues:
            context_parts.append(f"- {c.get('store')} | {c.get('valid_from')} to {c.get('valid_until')} | {c.get('pages')} pages | {c.get('products_count')} products")
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            return "Greška u servisu. Pokušajte ponovno."
        ...
        print(f"❌ chat_with_gemini exception: {e}")
...
def get_history(phone, limit=6):
    try:
        ...
        if not isinstance(data, list):
            return []
        # Return in chronological order with correct field names
...
def send_whatsapp(to, body, media_url=None):
    ...
    try:
        ...
        if r.status_code in [200, 201]:
            print(f"✅ Twilio sent to {to}")
            return True
        else:
            print(f"❌ Twilio error {r.status_code}: {r.text[:200]}")
            return False
    except Exception as e:
        print(f"❌ send_whatsapp exception: {e}")
        return False


# ─────────────────────────────────────────────
# DETECT IF USER WANTS A PAGE IMAGE
# ─────────────────────────────────────────────
def detect_page_request(message):
    """Returns (store, page_number) or (None, None)."""
    msg = message.lower()
    ...
    store = next((STORE_MAP[s] for s in STORES if s in msg), None)

    page_keywords = ["page", "stranica", "strana", "str", "slika", "sliku", "show", "prikaži"]
...
def webhook():
    ...
    print(f"\n📱 WhatsApp from {sender}: {incoming[:100]}")
    ...
    if not user:
        ...
        print(f"  👤 Existing user: {sender} (searches: {user.get('total_searches', 0)})")

    # Save incoming message
    save_message(sender, "user", incoming)

    # Get conversation history
    history = get_history(sender)

    # Check if user wants a specific page image
    store, page_num = detect_page_request(incoming)
    if store and page_num:
        image_url = get_page_image(store, page_num)
        if image_url:
            send_whatsapp(sender, f"{store} - stranica {page_num}:", media_url=image_url)
            save_message(sender, "assistant", f"[Page image sent: {store} page {page_num}]")
    ...
    # Smart search
    products, detected_store = search_products(incoming)
    catalogues = []

    # If asking about a catalog (not products)
    catalog_keywords = ["katalog", "catalog", "catalogue", "ponuda", "akcija", "offer"]
    if any(kw in incoming.lower() for kw in catalog_keywords):
        catalogues = search_catalogues(detected_store)

    # Generate AI reply
    reply = chat_with_gemini(incoming, products, catalogues, history)
